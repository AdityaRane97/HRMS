...
/// Supports CRUD operations with proper authorization.
/// Phase 1: Basic implementation. Phase 2: Add request/response validation.
/// </summary>
[ApiController]
...
public class EmployeesController : ControllerBase
{
    ...
    public EmployeesController(HrmsDbContext context, IMapper mapper, ILogger<EmployeesController> logger)
    {
        _context = context;
        _mapper = mapper;
        _logger = logger;
    ...
    /// Phase 1: Returns all employees. Phase 2: Add authorization filtering.
    /// </summary>
    [HttpGet]
    [ProducesResponseType(typeof(ApiResponse<PaginatedList<EmployeeDto>>), StatusCodes.Status200OK)]
    public async Task<ActionResult<ApiResponse<PaginatedList<EmployeeDto>>>> GetEmployees(
        [FromQuery] int pageNumber = 1,
        [FromQuery] int pageSize = 10,
        CancellationToken cancellationToken = default)
    {
        _logger.LogInformation("Getting employees - Page {PageNumber}, Size {PageSize}", pageNumber, cancellationToken);
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var existingEmployee = await _context.Employees
            .FirstOrDefaultAsync(e => e.Email == createDto.Email, cancellationToken);
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        _logger.LogInformation("Getting current user employee profile");

        // Phase 1: Placeholder. Phase 2: Get from claims/CurrentUser service
        var userId = Guid.TryParse(currentUser.FindFirst("sub")?.Value, out var parsedId)
            ? parsedId
            : Guid.Empty;
